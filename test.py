...
def fetch_jpm_akshare_standard() -> pd.DataFrame:
    """
    AKShare官方标准美股日线接口，替代已废弃的stock_us_daily_sina
    数据源：新浪财经，支持前复权/未复权，完全覆盖2018-2024年历史数据
    """
    print("\n==================== 开始拉取AKShare标准JPM美股数据 ====================")

    try:
        # 1. 拉取全量历史日线数据
        # adjust="qfq" 返回前复权数据（回测推荐），adjust="" 返回原始未复权数据
        df = ak.stock_us_daily(symbol=TICKER, adjust="qfq")

        # 2. 数据清洗与格式对齐（和原yfinance输出格式完全一致）
        df["date"] = pd.to_datetime(df["date"])
        df.set_index("date", inplace=True)
        # 严格过滤项目要求的日期范围
        df = df.sort_index().loc[START_DATE:END_DATE]

        # 3. 列名标准化，匹配BSM模型后续计算要求
        df.rename(
            columns={
                "open": "Open",
                "high": "High",
        ...
        if df.empty or len(df) < 1000:
            raise ValueError(f"数据量异常，仅获取到{len(df)}条数据，不符合2018-2024年日线数据量要求")

        print(f"✅ 成功拉取JPM数据：共{len(df)}条交易日数据")
        print(f"📅 数据范围：{df.index.min().date()} 至 {df.index.max().date()}")
        print(f"📊 数据字段：{list(df.columns)}")

        # 5. 保存数据
        save_data(df, f"jpm_daily_akshare_{START_DATE[:4]}_{END_DATE[:4]}")
        return df

    except Exception as e:
        print(f"❌ AKShare标准接口拉取失败: {str(e)}")
        return None
...
if __name__ == "__main__":
    ...
    #fetch_jpm_alpha_vantage()
    fetch_jpm_akshare_standard()
    # 一键拉取所有第一周所需数据
